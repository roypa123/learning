// libaudio - wav_writer.cpp
// See Chapter 9 for the byte-by-byte explanation of this format.
...
// Clamp, scale by 32767 (not 32768), round to nearest. Chapter 9 s9.4.
              ...
              bool dither)
{
    ...
    writeTag  (out, "fmt ");                        // trailing space is required
    writeU32LE(out, 16);
    ...
    if (dither)
    {
        ...
        {
            const float noise = (dist(rng) - dist(rng)) / 32768.0f;   // TPDF
              ...
              bool dither)
{
    ...
    return writeWav(path, interleaved, sampleRate,
                    static_cast<int>(numChannels), dither);
}

bool writeWav(const std::string& path, const AudioBuffer& buffer, bool dither)
{
    if (buffer.numChannels() < 1 || buffer.numFrames() < 1)
        return false;

    return writeWav(path, buffer.interleaved(),
                    static_cast<int>(buffer.sampleRate()),
                    buffer.numChannels(), dither);
}
